...
import org.bookyourshows.exceptions.SeatCreationException;
...
public class SeatUtils {
    ...
    public static void validateSeatCreateRequests(List<SeatCreateRequest> requests) throws CustomException {
        ...
        if (requests == null || requests.isEmpty()) {
            throw new SeatCreationException("Seat request list cannot be empty");
        }
        ...
        for (SeatCreateRequest request : requests) {
            ...
            if (request.getRowNo() == null) {
                throw new SeatCreationException("row_no is required");
            }
            if (request.getRowNo() <= 0) {
                throw new SeatCreationException("row_no must be greater than 0");
            }
            if (!seenRows.add(request.getRowNo())) {
                throw new SeatCreationException("Duplicate row_no: " + request.getRowNo());
            }

            int totalSegmentsInRow = getTotalSeatsInRow(request);

            if (totalSegmentsInRow > 100) {
                throw new SeatCreationException("Total seats in row " + request.getRowNo() + " cannot exceed 100");
            }
        }
    }
    ...
    private static int getTotalSeatsInRow(SeatCreateRequest request) throws CustomException {
        if (request.getSegments() == null || request.getSegments().isEmpty()) {
            throw new SeatCreationException("segments cannot be empty for row " + request.getRowNo());
        }
        ...
        for (SeatSegments segment : request.getSegments()) {
            ...
            if (segment.getSeatCategoryId() == null) {
                throw new SeatCreationException("seat_category_id is required in row " + request.getRowNo());
            }
            if (segment.getSeatCount() == null) {
                throw new SeatCreationException("seat_count is required in row " + request.getRowNo());
            }
            if (segment.getSeatCount() <= 0) {
                throw new SeatCreationException("seat_count must be greater than 0 in row " + request.getRowNo());
            }
            if (segment.getSeatCount() > 50) {
                throw new SeatCreationException("seat_count cannot exceed 50 in row " + request.getRowNo());
            }
            if (!seenCategories.add(segment.getSeatCategoryId())) {
                throw new SeatCreationException("Duplicate seat_category_id " + segment.getSeatCategoryId() + " in row " + request.getRowNo());
            }
